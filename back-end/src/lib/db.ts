 ...
 */
export async function connectDB(retries: number = 3, delay: number = 2000): Promise<Db> {
  if (db) return db;
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.error(`❌ MongoDB Connection Error (Lần thử ${i + 1}/${retries}):`, error.message);
      ...
      if (i < retries - 1) {
        ...
        console.error("   - Mac/Linux: brew services start mongodb-community hoặc sudo systemctl start mongod");
...
export const getDb = (): Db | null => {
  if (!db) {
    console.error('Database chưa được kết nối. Vui lòng gọi connectDB() trước.');
    return null;
  }
  return db;
};